...
export default function ListingsPage() {
  console.log('entering listings page');
  const { storefront, ...store } = useStore();
  console.log('listings page, got store', { storefront, store });
  //   const [searchParams, setSearchParams] = useSearchParams();
  const router = useRouter();
  const setViewQueryParam = (view: string) => {
  ...
  useEffect(() => {
    ...
    setViewQueryParam(view || View.live);
    // setSearchParams({
    //   view: view || View.live,
    // });
    // auto-forwards to first listings view with matching listings
    if (currentViewIsEmpty()) {
      setViewQueryParam(views.find(x => x.count() > 0)?.key || View.live);
      //   setSearchParams({
          ...
          {views.map(({ key, title, count }) => {
            return (
              <button
                ...
                onClick={() => setViewQueryParam(key)} // setSearchParams({ view: key })}
                disabled={count() === 0}
              >
